...
| 23 | Repo name, **reversing #3** | Rename the GitHub repo to `rutter` so the marketplace, the plugin and the repository all carry one name. #3 kept the repo to preserve history, stars and releases; GitHub's permanent redirect on rename preserves all three, and the split name was surfacing in the README's own install instructions — a stranger was being told to add a `meridian` marketplace to get a `rutter` plugin. `plugin.json`'s repository field and the CHANGELOG release links move with it. | J |
...
3. [x] transit vendored, speaking the vocabulary `[agent]` — delivers: transit loads and its labels and agent references match the shared vocabulary — acceptance: `details` lists 2 skills; probe confirms transit resolves `meridian:*` labels and names `rutter:<agent>` — seams: loaded plugin — blocked by: #2 — from decisions #2 #17 #22
4. [x] Six agents vendored and namespaced `[agent]` — delivers: the agent roster ships and resolves under the plugin namespace, free of origin-project text — acceptance: `details` shows Agents (6); probe lists `rutter:scout` and peers; no description names the origin repo — seams: loaded plugin — blocked by: #1 — from decisions #4 #17
5. [x] convoy vendored, guard relocated `[agent]` — delivers: convoy loads and its spawn guard runs from the plugin's own binary directory against the namespaced roster — acceptance: `details` lists 3 skills; `validate` passes with the binary directory present; guard refuses a spawn outside the roster — seams: loaded plugin — blocked by: #3 #4 — from decisions #2 #17 #18 #22
6. [x] meridian names both exits `[agent]` — delivers: meridian states the handoff rule — one ticket to transit, a whole map to convoy — acceptance: probe asking meridian where a route item goes names both and the rule for choosing — seams: loaded plugin — blocked by: #5 — from decision #11
7. [x] Command discovery replaces hardcoded build commands `[agent]` — delivers: the skills find a repo's build and test commands instead of assuming one toolchain — acceptance: probe in a makefile-only fixture returns that repo's commands; no skill or agent contains a hardcoded package-manager invocation — seams: loaded plugin — blocked by: #5 — from decision #5
8. [x] Eval suites relocated and running `[agent]` — delivers: each skill's behaviour is covered by a suite the plugin's eval runner can execute — acceptance: suites for all three skills run and report — seams: loaded plugin — blocked by: #5 — from decision #14
9. [x] Vocabulary and decoupling lint in CI `[agent]` — delivers: source-level drift fails the build — acceptance: a planted foreign label and a planted hardcoded build command both fail the check — seams: CI lint (not the seam) — blocked by: #7 — from decisions #1 #5
10. [x] Drift check against the origin project's copies `[agent]` — delivers: divergence between the two copies is caught mechanically — acceptance: a planted divergence fails loudly — seams: CI lint (not the seam) — blocked by: #8 — from decisions #6 #13
11. [x] README and docs for a three-skill plugin `[human]` — delivers: install, the trio and their handoffs, the vocabulary, and the unsupported distribution channel stated plainly — acceptance: a reader can install and chart a first map without reading a SKILL.md — seams: human review — blocked by: #6 #7 — from decisions #3 #18
12. [x] Agent safety pass (pre-publish gate) `[human]` — delivers: the roster's tool grants reviewed against an arbitrary repository — acceptance: every agent's grants justified or narrowed; findings recorded — seams: human review — blocked by: #4 #5 — from decision #12 — **flagged**: raised at the quiz that this gates publication (#13), not the vendoring; kept as drawn on the user's call, so it can be done early and go stale before release
13. [ ] Release 1.0.0 `[human]` — delivers: the plugin published at version parity with a tag and release — acceptance: CHANGELOG, plugin and marketplace versions agree; tag and GitHub release exist — seams: the release workflow's own checks — blocked by: #9 #10 #11 #12 #14 #15 #16 — from decision #7
14. [x] `auditor` made installable-anywhere `[agent]` — delivers: the audit agent works in a repo that has no `interface` skill and no `DESIGN.md` — acceptance: auditor's frontmatter no longer hard-requires a skill this plugin does not ship; its authority list degrades to whatever the repo actually has, and says so when it has none — seams: loaded plugin — blocked by: — — from decisions #2 #4 — **surfaced during item 4**: auditor's frontmatter requires the `interface` skill and its checklist cites that skill's `reference/review.md`; `interface` lives in the origin project and is not among the three skills shipping here, so as vendored the agent's stated authority does not exist for any installer. Filed rather than absorbed per convoy.
15. [x] convoy's spawn guard actually registers when installed `[human]` — delivers: the roster guard enforces for an installer, not just in the origin project — acceptance: with rutter installed, a non-roster `subagent_type` during a convoy is refused, and no spawn outside a convoy is affected — seams: loaded plugin — blocked by: — — from decisions #12 #18 — **surfaced during item 5**: convoy declares its PreToolUse guard in SKILL.md frontmatter. `details` reports `Hooks (0)`, and no shipped plugin in the official marketplace declares frontmatter hooks — all use `hooks/hooks.json`, which does report. So the guard script is correct and relocated but probably never fires from a plugin. The naive fix (move it to `hooks/hooks.json`) makes it always-on and would block every non-roster Agent spawn in any session where rutter is enabled, including `Explore` and `general-purpose` — so it needs an active-convoy condition, which is a design change. `mode:human`: needs a real install to verify.
16. [ ] Eval suites that discriminate `[agent]` — delivers: every case scores higher with the plugin than without — acceptance: no case has a zero or negative delta; no criterion passes or fails in both arms — seams: `claude plugin eval` — blocked by: — — from decision #14 — **materially improved, bar not met.** Two root causes found and fixed: the eval sandbox workspace is empty, so every case that named a `plans/` file was asking the agent to read something that did not exist (maps are now embedded inline in the prompt), and 8 criteria that passed in both arms were dropped. Current state, 8 cases x 3 runs x 2 arms: 5 discriminate (`fogbound-bearings-pass` +0.58, `afk-label-gate` +0.33, `research-cites-and-defers` +0.33, `workspace-defaults-to-file` +0.11, `slices-trace-to-decisions` +0.07); 3 show the plugin scoring BELOW baseline (`route-gate-holds` -0.44, `waves-gates-and-commits` -0.11, `judgments-resolved-not-parked` -0.07); 15 criteria still pass or fail in both arms. The negative deltas look like criteria punishing correct behaviour rather than a skill defect — `route-gate-holds` asserts 'no slices before approval', and given a route-ready map meridian proceeds to chart the route, which is its job; the baseline passes by merely describing what it would do. That is the third time in this item that a criterion has scored correct behaviour as failure, which is the actual difficulty: writing single-shot criteria for an interactive planning skill without punishing it for acting. At 3 runs per arm the deltas are also noisy — `fogbound` moved 0.67→0.75 with and 0.27→0.17 without between two runs of the same suite. Raise runs before trusting any single delta. Roughly $12 and nine validation runs spent here so far.
