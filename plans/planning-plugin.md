...
16. [ ] Eval suites rewritten for a single-shot harness `[agent]` — delivers: cases that discriminate, i.e. score above zero with the plugin and below it without — acceptance: no case scores 0.00 in both arms; every criterion passing in both arms is replaced or dropped — seams: `claude plugin eval` — blocked by: — — from decision #14 — **diagnosed, not calibrated**: two hypotheses were tested and rejected. (a) Tool grants: the prompts granted no Write, so meridian could not record a map — fixed, moved the case 0.00 → 0.08 and no further. (b) Triggering: the prompts were bare user requests, so the case tested autonomous activation as well as behaviour — every prompt now names its skill, which moved the score not at all. Runs are healthy (9 of 20 turns, no error), and the same case scores 0.00–0.25 across runs, so the cases are non-deterministic against their own criteria. The remaining explanation is that these cases were authored for a harness that force-loaded the skill and graded per-assertion, and they describe things a single-shot run does not reliably produce — chiefly an interactive checkpoint prompt (`(a)ccept / (k)eep grilling`) that exists to stop and wait for a user who is not present, and a map file written to a sandbox. That is a rewrite against what this harness can observe, not a tuning of what is there. Next diagnostic step is one run with `--keep-temp` and a read of `trace.jsonl` to see the actual output before rewriting anything. Cost is ~$0.2 and ~3 minutes per validation cycle.
