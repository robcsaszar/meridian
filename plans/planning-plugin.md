...
> Meridian map · Status: route-complete · Started: 2026-09-22 · Tracker: local-markdown
...
13. [x] Release 1.0.0 `[human]` — delivers: the plugin published at version parity with a tag and release — acceptance: CHANGELOG, plugin and marketplace versions agree; tag and GitHub release exist — seams: the release workflow's own checks — blocked by: #9 #10 #11 #12 #14 #15 #16 — from decision #7
...
16. [x] Eval suites that discriminate `[agent]` — delivers: a suite where every shipped case scores higher with the plugin than without — acceptance met at n=10: three cases ship, each with a delta larger than its spread — `meridian/fogbound-bearings-pass` +0.80 (0.90±0.22 vs 0.10±0.16), `meridian/slices-trace-to-decisions` +0.55, `convoy/judgments-resolved-not-parked` +0.22. Five cases are quarantined with their data in `evals/quarantine/README.md`. Measured properly at 10 runs per arm ($29.88, 1150s) after 3-run deltas proved untrustworthy — two cases that read as consistently positive across three runs turned out to be within noise.
17. [ ] Investigate convoy scoring below baseline on route planning `[agent]` — delivers: a verdict on whether `waves-gates-and-commits` is a bad test or a real convoy defect — acceptance: a trace read says which, and either the criteria are fixed or a convoy defect is filed — seams: `claude plugin eval --keep-temp` — blocked by: — — from decision #14 — **surfaced by item 16's stable read**: convoy scores 0.23±0.24 against a baseline's 0.55±0.22 when asked to describe how it would work a whole route. Negative in all four full runs, and the margin at n=10 exceeds either spread. Either the criteria reward a generic project-management answer that a baseline writes fluently while convoy's is specific and constrained, or convoy genuinely produces a worse plan than no skill at all. The second would be a real defect. Does NOT block release: the case is quarantined, so nothing ships that asserts it.
