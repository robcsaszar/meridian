...
16. [ ] Eval suites rewritten for a single-shot harness `[agent]` — delivers: cases that discriminate — acceptance: no case scores 0.00 in both arms; every criterion passing in both arms is replaced or dropped — seams: `claude plugin eval` — blocked by: — — from decision #14 — **rewrite done and proven; validation of the remainder outstanding.** Diagnosed from a `--keep-temp` trace: the skill was behaving correctly and the cases were wrong. meridian produced a map, a candidate Destination, a recommendation with its weakness named and exactly two orienting questions including appetite — textbook Phase 1 — and scored zero because the criteria demanded the `(a)ccept / (k)eep grilling` checkpoint that only arrives after the user answers those questions. The cases described a conversation. transit's legacy case also asserted a pre-plugin path (`.claude/skills/meridian/...`) that no longer exists, so it could never pass. All 8 cases were rewritten against what one shot can observe, each criterion a stated skill rule a baseline has no reason to follow. Two are validated with ablation: `meridian/fogbound-bearings-pass` with 0.53 / without 0.13 (Δ +0.40) and `transit/workspace-defaults-to-file` with 0.73 / without 0.47 (Δ +0.27). Remaining: validate the other six (~$0.8 and ~3 minutes each) and drop the criteria that pass in both arms — `no-invented-timeline`, `does-not-adopt-a-tracker-from-repo-signals`, `no-tracker-writes` and `one-ticket-then-stops` are already known to be non-discriminating. One case was dropped rather than rewritten: transit's PR-cap gate needs live GitHub state a sandbox cannot supply.
