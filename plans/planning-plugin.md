...
16. [ ] Eval suites calibrated so they discriminate `[agent]` — delivers: each case scores above zero with the plugin and shows a real delta against the no-plugin arm — acceptance: no case scores 0.00 in both arms; every criterion that passes in both arms is replaced or dropped per the discrimination table — seams: `claude plugin eval` — blocked by: — — from decision #14 — **surfaced during item 8**: the suites were converted from an evals.json format graded by a different harness and they run, but the one case exercised scored 0.00 in both arms across six runs. First cause found and fixed in item 8 (the prompts granted no Write, so meridian could not record a map at all); whether that alone fixes the score is unverified, and a suite that fails in both arms tests nothing. Each validation run costs roughly $0.2 and three minutes, so calibration is its own item rather than an open-ended loop inside item 8.
